...
async fn start_tracking(
    ...
    let (
        ...
        if already_tracking {
            ...
            s.is_tracking = true;
            // Gate only the immediate post-start snapshot. If the first track arrives later,
            // treat it as a real live event.
            s.awaiting_first_live_change = true;
            s.tracking_started_at = Some(std::time::Instant::now());
...
async fn save_config(
    ...
    let software_changed = {
        ...
        let software_changed = s.config.dj_software != config.dj_software;
