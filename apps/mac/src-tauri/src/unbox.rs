...
const START_SNAPSHOT_GRACE_MS: u128 = 1800;
...
async fn handle_track_change(
    ...
    // Ignore only the immediate post-start snapshot. If the first track arrives
    // later than the grace window, treat it as a real live event.
    if s.awaiting_first_live_change {
        let within_start_grace = s
            .tracking_started_at
            .map(|started_at| started_at.elapsed().as_millis() <= START_SNAPSHOT_GRACE_MS)
            .unwrap_or(false);

        if within_start_grace {
            println!("[TrackCast][track] ignoring immediate post-start snapshot");
