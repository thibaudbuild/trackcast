...
fn traktor_app_candidates() -> [PathBuf; 2] {
    [
...
fn traktor_csi_dir() -> Option<PathBuf> {
    traktor_app_path().map(|path| path.join("Contents/Resources/qml/CSI"))
}

fn installed_helper_dir() -> Option<PathBuf> {
    traktor_csi_dir().map(|path| path.join(HELPER_DIR_NAME))
}

fn installed_helper_file() -> Option<PathBuf> {
    installed_helper_dir().map(|path| path.join(HELPER_MAIN_FILE))
}

fn installed_version_file() -> Option<PathBuf> {
    installed_helper_dir().map(|path| path.join(VERSION_MARKER_FILE))
}
...
pub async fn get_traktor_setup_status() -> TraktorSetupStatus {
    let plugin_files_present = installed_helper_file().is_some_and(|path| path.exists());
...
pub fn open_traktor_csi_folder() -> Result<String, String> {
    let csi = traktor_csi_dir().ok_or_else(|| {
        "Traktor Pro app not found (checked Pro 4 and Pro 3 in /Applications/Native Instruments)."
            .to_string()
    })?;
    if !csi.exists() {
        return Err(format!(
...
pub fn install_traktor_helper(app_handle: &tauri::AppHandle) -> Result<String, String> {
    let app_path = traktor_app_path().ok_or_else(|| {
        "Traktor app not found (checked Pro 4 and Pro 3 in /Applications/Native Instruments)."
            .to_string()
    })?;

    let source_dir = resolve_bundle_helper_dir(app_handle)?;
    let target_csi = traktor_csi_dir().ok_or_else(|| {
        format!(
            "Traktor CSI folder not found under {}",
            app_path.display()
        )
    })?;
    let target_helper = installed_helper_dir().ok_or_else(|| {
        format!(
            "Traktor helper target folder not resolved under {}",
            app_path.display()
        )
    })?;
    ...
    let version_file = installed_version_file().ok_or_else(|| {
        "Cannot resolve Traktor helper version marker location.".to_string()
    })?;
    fs::write(version_file, HELPER_VERSION)
        .map_err(|e| format!("Helper installed but version marker failed: {}", e))?;

    Ok("Traktor helper installed. Open Traktor Pro 4/3, select D2 in Controller Manager, then click Verify.".to_string())
}
...
pub async fn ensure_traktor_ready_or_err() -> Result<(), String> {
    ...
    Err("Traktor setup incomplete — API offline. Verify after selecting D2 in Traktor Pro 4/3 Controller Manager.".to_string())
}
